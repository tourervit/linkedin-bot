  ...
  function(message, sender, sendResponse) {
    if (message.code == 'sn.startFollowUp') {
      ...
      function checkLastMessageInChat(chatList) {
        return new Promise((resolve) => {
          ...
          $.each(chatList, (index, item) => {
            ...
            if (chatLastMessage.toLowerCase().indexOf('you:') > -1 && chatLastMessage.toLowerCase().indexOf(testString.toLowerCase()) > -1) {
              chatsMap.push($chat);
            }
          });
          resolve(chatsMap);
        ...
        return new Promise(resolve => {
          ...
          function write() {
            return new Promise(resolve => {
              ...
              const $messageField = $('.msg-compose-form__message-text');
              const $btnSend = $('.msg-compose-form__send-button');
              setTimeout(() => {
                $(".msg-compose-form__message-text").sendkeys(messageForSend);
              }, 2000);
              setTimeout(() => {
                $btnSend.click();
                numberOfMessages++;
      ...
      function initBot() {
        checkChats()
          .then(chatsList => {
            checkLastMessageInChat(chatsList)
              .then(chatsMap => {
                if (chatsMap.length) {
